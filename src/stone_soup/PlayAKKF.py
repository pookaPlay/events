...
# Also import a standard Kalman predictor/updater and GaussianState to run a baseline
...
linear_measurement_model = LinearGaussian(
    ndim_state=4,
    mapping=(0, 2),
    # 2D measurement noise covariance (x and y)
    noise_covar=np.array([[0.005 ** 2, 0.0], [0.0, 0.005 ** 2]])
    )
    #ndim_state=4,  # Number of state dimensions (position and velocity in 2D)
    #mapping=(0, 2),  # Mapping measurement vector index to state index
    #noise_covar=np.array([[5, 0],  # Covariance matrix for Gaussian PDF
    #                      [0, 5]])
# Run a standard Kalman filter (linear measurement) on the same measurements to compare
# Build Kalman predictor/updater using the same transition and measurement models
kf_predictor = KalmanPredictor(transition_model)
kf_updater = KalmanUpdater(linear_measurement_model)

# Initialise a Gaussian prior for the Kalman filter using the ground truth initial state
init_mean = np.asarray(truth[0].state_vector).reshape((4, 1))

kf_prior = GaussianState(init_mean, np.diag([-0.3, 0.001, 0.7, -0.055]), timestamp=start_time)
# [1.5, 0.5, 1.5, 0.5] => [-0.3, 0.001, 0.7, -0.055]

# Generate linear (position) measurements from the truth using the linear model
linear_measurements = []
for state in truth:
    lm = linear_measurement_model.function(state, noise=True)
    linear_measurements.append(Detection(lm,
                                         timestamp=state.timestamp,
                                         measurement_model=linear_measurement_model))

# Run KF over linear measurements (position measurements)
kf_track = Track()
for measurement in linear_measurements:
    kf_prediction = kf_predictor.predict(kf_prior, timestamp=measurement.timestamp)
    kf_hypothesis = SingleHypothesis(kf_prediction, measurement)
    kf_post = kf_updater.update(kf_hypothesis)
    kf_track.append(kf_post)
    kf_prior = kf_track[-1]
...
plotter = Plotter()
# Plot the linear (position) measurements used by the KF for comparison
plotter.plot_measurements(linear_measurements, [0, 2])
...
plotter.plot_tracks(kf_track, [0, 2], label='KF - linear', color='orangered', uncertainty=True)
plotter.fig
plt.show()
#.show()
...
plotter.plot_tracks(kf_track, [0, 2], particle=False)
plotter.fig.show()
plt.show()
